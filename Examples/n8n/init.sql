...
CREATE INDEX IF NOT EXISTS idx_reminders_active_location ON reminders(status, trigger_type)
    WHERE status = 'active' AND trigger_type IN ('location', 'hybrid');
...
DO $$
BEGIN
    RAISE NOTICE 'Reminder Bot database initialized successfully!';
END $$;
